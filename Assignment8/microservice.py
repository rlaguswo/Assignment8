...
try:
    img_counter = set_init(IMAGECOUNTER_FILENAME)
   
    while True:
            try:
                update_img = file_check_update(IMAGEFILENAME)
                if update_img:
                    # Increase counter for filename and update counter
                    img_counter += 1
                    with open(IMAGECOUNTER_FILENAME, "w") as file:
                        file.write(str(img_counter))
                    set_file(IMAGECOUNTER_FILENAME, IMAGEFILENAME, IMAGENAME, IMAGEEXTENSION, img_counter)
